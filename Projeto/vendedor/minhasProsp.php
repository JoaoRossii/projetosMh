...
$sql = $sql = "SELECT * FROM anuncios join cadastrousuario on fkUsuario = id WHERE fkVendor = '$id'"; /* query utilizada para buscar dados no banco para exibir em um card */
    ...
    <?php
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                echo "<form action='main-tela-prosp.php' method='POST'>";
                ...
                echo "<button type='submit' class='acceptButton'>Comprar</button> ";
                echo "</div> ";
                echo "</div> ";
                echo "</form>";
            } 
        } else {
                echo "Você não possui nenhum anuncio.";
            }
        ?>
